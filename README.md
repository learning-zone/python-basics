...
```python
    a = [1, 2, 3, 4]
    # first approach using slicing
    a[::-1]

    # second approach using sort function
    a.sort(reverse=True)

    # third approach using sorted function
    sorted(reverse=True)
    # sorted function is not permanent unless you reassigned the result to a variable.
```
...
you can also use the **extend** function to merged two sorted lists they behaves exactly the same.
...
Dividing value by zero will return ZeroDvisionError
...
#### Q. Differentiate between "_.py" file and "_.pyc" file?
...
append - will add element to a list last index
...
```python
    ...
    # result [10, 25, 31, 50, {'key': 'value'}]

```

extend - extend merged two list together. unlike the append function passing dictionary as an argument will only merged its key and ignored the value assigned to it.
...
ord() - return ASCII version of the character you passed as an argument

```python
    >>> ord('C')
    >> 67
    # result : 'C' if you take a look to ASCII table
    # 'C' is indeed 67
```

chr() - meanwhile chr() is the opposite of ord() it returns a character that equivalent to the value you pass as an argument.
